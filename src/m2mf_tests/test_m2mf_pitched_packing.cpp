#include "test_m2mf_pitched_packing.h"
...
bool TestM2MFPitchedPacking::Test() {
  LogMsg("\n============================================================\n");
  LogMsg("Test: M2MF Pitched Row Packing/Unpacking\n");
  ...
  constexpr size_t kMaxSpan = 4096;
  ...
  const PackingCase cases[] = {
      {64, 4, 64, 128, "Unpacking rows (dense src -> pitched dst)"},
      {64, 4, 128, 64, "Packing rows (pitched src -> dense dst)"},
  };
  ...
  for (const auto& tc : cases) {
    ...
    uint8_t seed = static_cast<uint8_t>(tc.pitch_in ^ tc.pitch_out ^ 0x5A);
